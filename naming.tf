...
#------------------------------------------------------------
data "popsrox_utils_resource_name" "diagnostic_setting" {
  ...
  suffixes      = compact([var.name_prefix == "" ? null : local.name_prefix, var.deploy_environment, local.name_suffix, var.use_naming ? "" : "diagsetting"])
